...
use crate::model::{LanguageModel, config::{ModelConfig, TensorWeightsConfig}, gpt2::GPT2, qwen3::Qwen3};
...
impl ModelBuilder {
    ...
    pub async fn build(self) -> Box<dyn LanguageModel> {
        ...
        match &self.config.qwen {
            true => {
                Box::new(GPT2::new(&tensor_weights, self.config.use_gpu).await.unwrap())
            }
            false => {
                Box::new(Qwen3::new(&tensor_weights, self.config.use_gpu).await.unwrap())
            }
        }
    }
}